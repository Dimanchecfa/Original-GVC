...
const AllHistory = () => {
   ...
   const fetchLastMonthMoto = async () => {
    alertPending();
       await HTTP_CLIENT.get("http://localhost:8000/api/history/lastmonth/moto_number")
              .then((response) => {
    ...
    const fetchLastMonthPrice = async () => {
        await HTTP_CLIENT.get("http://localhost:8000/api/history/lastmonth/price")
                .then((response) => {
                    const data = response.data?.data;
                    setPrice(data);
  ...
  return (
   <>
    ...
    <div class="col-md-4  offset-md-2">
        ...
        <Card title="Nombre de moto ecoule pour le mois dernier " svg={<i class="fas fa-2x fa-car"></i>} route="/admin/stock/vente" state="vente" data={ moto }button="Détails" />
        <Card title="Somme encaisse pour le mois dernier" svg={<i class="fas fa-2x fa-usd"></i>} route="/admin/stock/vente"  data={ price} button="Details" unite= "FCFA" />
      </div>
